...
echo "${myname/work/com}"
    #+end_src
12. ~${parameter:-word}~ If value of ~parameter~ if null, then return ~word~.
13. ~${parameter:=word}~ If value of ~parameter~ if null, then set ~word~ to ~parameter~, and return ~word~.
14. ~~
*** Example
#+begin_src shell
for i in {1..100}; do
